 ...
 * 날짜와 시간 데이터 형식
 * Date : 일반형식. 초단위까지 표시
 * Timestamp : 표준시를 기준으로 하는 timezone 표시할 수 있고 . 
 * 			  나노(초이하 9자리)초 까지 표시 가능
 * 
 * 일반적으로는 date와 timestmap를 표시하는 방법은 동일하다
 */
...
--내일 시험에서 timestemp타입은 date타입과 동일하게 코딩하면 됨
-- 날짜형식은 문자열 'yyyy--MM--dd'과 자동캐스팅이 됩니다.
-- date와 timestamp 모두 to_char함수는 동일하게 사용가능.
